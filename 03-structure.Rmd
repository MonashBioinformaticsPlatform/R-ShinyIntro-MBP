...
This is the front-end of the shiny app that allows you to capture (input) and display (output) a variety of data types. Fundamentally the UI is an HTML document with shiny functions returning chunks of HTML code. 
...
Typically, we don't alter the session object as its default behavior is adequate for most needs. The session object is an environment that can be used to access information and functionality relating to the session. For more information, [check the documentation here](https://shiny.rstudio.com/reference/shiny/latest/session.html)