...
Building a UI involves nesting a lot of components inside each other and this can be overwhelming to look at when you are getting started. It's quite easy to lose track of where your closing brackets should be and quite easy to have a misplaced comma that prevents your app from running. We can't emphasis this enough - getting the nesting right can prevent a lot of problems with your shiny app not running. As you become more experienced with writing Shiny, you'll get used to examining your UI element code with an eagle eye to make sure you haven't misplaced a comma or bracket. 

From the example above, we can see that `sidebarPanel` and `mainPanel` are nested within `sidebarLayout`. `sidebarLayout` itself is also nested within `fluidPage`. As more components are added to the app, e.g widgets and outputs, they too will be nested within the panel functions to control their location on the layout.
...
Does the `tabsetPanel` need to be nested in the `sidebarLayout`? 
...
The UI also contains elements for displaying output results. The list of this are not as expansive as the widgets and can be [found here (under UI outputs)](https://shiny.rstudio.com/reference/shiny/1.7.0/). The type of result you want displayed in your application determines the type of UI element that you need.
