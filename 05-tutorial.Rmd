...
Create one blank R script (Click `File` > `New File` > `R Script`). Save it inside the `demo` folder with the name `app.R`. 

Now we're going to create a blank app. In app.R, type:
...
6. For an ever further challenge, create a reactive function and pass the reactive variable to the two `render` functions. Read the csv file only once in the application
